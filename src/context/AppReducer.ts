    ...
    switch (action.type) {
        ...
        case "SET_MONTHLY_INCOME": {
            ...
            const updated = existing
                ? state.budget.monthlyIncomes.map(m => m.month === action.payload.month ? action.payload : m)
                : [...state.budget.monthlyIncomes, action.payload]
            return { ...state, budget: { ...state.budget, monthlyIncomes: updated } }
        }
        ...
        case "RESET_INCOME":
            return { ...state, budget: { ...state.budget, monthlyIncomes: [] } }
